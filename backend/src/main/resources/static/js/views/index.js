...
class InicioView {
    ...
    constructor() {
        ...
        this.dom.querySelector('#searchForm').addEventListener('submit', this.search);
        this.dom.querySelector('#btnLimpiar').addEventListener('click', this.limpiar);
        this.load();
    }
    ...
    load = async () => {
        ...
        const sel = this.dom.querySelector('#caracteristica');
        (data.caracteristicas || []).forEach(c => {
            const o = document.createElement('option');
            o.value = c.id; o.textContent = c.nombre; sel.appendChild(o);
        });
        this.renderGrid(data.puestosRecientes || []);
    }

    search = async (e) => {
        e.preventDefault();
        const params = new URLSearchParams();
        const p = this.dom.querySelector('#palabraClave').value;
        const c = this.dom.querySelector('#caracteristica').value;
        const s = this.dom.querySelector('#salarioMin').value;
        ...
        this.dom.querySelector('#seccionTitulo').textContent =
            `Resultados de búsqueda (${res.length} encontrados)`;
        this.renderGrid(res);
    }

    limpiar = async () => {
        this.dom.querySelector('#searchForm').reset();
        const { data } = await apiGet('/api/publico/puestos/buscar');
        this.dom.querySelector('#seccionTitulo').textContent = 'Puestos Publicados Recientemente';
        this.renderGrid(data.puestosRecientes || []);
    }

    renderGrid = (puestos) => {
        const grid = this.dom.querySelector('#puestosGrid');
        const sinRes = this.dom.querySelector('#sinResultados');
        grid.innerHTML = '';
...
function vistaInicio() {
    app().innerHTML = '';
    app().appendChild(new InicioView().dom);
}
