// Vista: Habilidades Oferente
class OferenteHabilidadesView {
    dom;
    state;

    constructor() {
        this.state = { habilidades: [], caracteristicas: [] };
    ...
    load = async () => {
        ...
        this.state.habilidades = data.habilidades || [];
    ...
    renderContent = () => {
        const opts = this.state.caracteristicas.map(c =>
            `<option value="${c.id}">
                ${c.parent ? c.parent.nombre + ' > ' + c.nombre : c.nombre}
             </option>`
        ).join('');

        const filas = this.state.habilidades.map(h => `
                ...
                <div class="section-card" style="margin-bottom:24px;">
                    ...
                    <form class="auth-form" id="habForm">
                        ...
                        <p style="font-size:15px;color:#888888;margin-top:-8px;">
                            ...
                            Si la habilidad ya existe, se actualizará el nivel.
                        </p>
                    </form>
                </div>
    ...
    bindEvents = () => {
        this.dom.querySelector('#habForm').addEventListener('submit', this.agregar);
        this.dom.querySelectorAll('.btn-eliminar').forEach(btn => this.bindEliminar(btn));
    }

    agregar = async (e) => {
        e.preventDefault();
        const caractId = this.dom.querySelector('#caracteristicaId').value;
        const nivel    = this.dom.querySelector('#nivel').value;
        const { ok: o, data: d } = await apiPost('/api/oferente/habilidades/agregar', {
            caracteristicaId: caractId, nivel
        });
        const msg = this.dom.querySelector('#msgHab');
        if (!o) { msg.innerHTML = alerta(d.error); return; }

        const sel    = this.dom.querySelector('#caracteristicaId');
        ...
        this.dom.querySelectorAll('#tablaHabilidades tbody tr').forEach(tr => {
        ...
        if (filaExistente) {
            ...
            let tbody = this.dom.querySelector('#tablaHabilidades tbody');
            if (!tbody) {
                ...
                tbody = this.dom.querySelector('#tablaHabilidades tbody');
            }
            ...
            this.bindEliminar(tr.querySelector('.btn-eliminar'));
            msg.innerHTML = alerta('Habilidad agregada.', 'success');
        }
        this.dom.querySelector('#habForm').reset();
    }
...
function vistaOferenteHabilidades() {
    app().innerHTML = '';
    app().appendChild(new OferenteHabilidadesView().dom);
}
