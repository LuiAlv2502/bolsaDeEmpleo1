...
class AdminPanelView {
    ...
    constructor() {
        ...
        // Leer querystring del hash para actualId
        const hash = location.hash;
        const qs = hash.includes('?') ? hash.split('?')[1] : '';
        this.state.actualId = new URLSearchParams(qs).get('actualId') || '';
    ...
    load = async () => {
        const url = '/api/admin/panel' + (this.state.actualId
            ? `?actualId=${encodeURIComponent(this.state.actualId)}` : '');
        ...
        if (!ok) {
            if (status === 401) {
                ...
                <main class="auth-main">
                    <div class="auth-card">
                        <h2>No autorizado</h2>
                        <p style="color:#c0392b;">Debe iniciar sesión como administrador.</p>
        ...
        this.dom.innerHTML = this.renderContent(data);
    ...
    bindEvents = (data) => {
        ...
        this.dom.querySelectorAll('.btn-aprobar[data-tipo]').forEach(btn => {
            btn.addEventListener('click', async () => {
                ...
                if (o) {
                    ...
                    this.dom.querySelector('#msgAdmin').innerHTML = alerta(d.error);
                }
            });
        });

        // Eliminar característica
        this.dom.querySelectorAll('.btn-eliminar[data-caract-id]').forEach(btn => {
            btn.addEventListener('click', async () => {
                const { ok: o, data: d } = await apiDelete(`/api/admin/caracteristica/${btn.dataset.caractId}`);
                if (o) btn.closest('tr').remove();
                else this.dom.querySelector('#msgAdmin').innerHTML = alerta(d.error);
            });
        });

        // Nueva característica
        this.dom.querySelector('#caracForm').addEventListener('submit', async (e) => {
            e.preventDefault();
            const nombre  = this.dom.querySelector('#caracNombre').value;
            const padreId = this.dom.querySelector('#caracPadreId').value || null;
            const { ok: o, data: d } = await apiPost('/api/admin/caracteristica/nueva', { nombre, padreId });
            if (!o) { this.dom.querySelector('#msgAdmin').innerHTML = alerta(d.error); return; }

            const tbody = this.dom.querySelector('.panel-table.tabla-margin tbody');
            ...
            tr.querySelector('.btn-eliminar').addEventListener('click', async () => {
                ...
                if (oe) tr.remove();
                else this.dom.querySelector('#msgAdmin').innerHTML = alerta(de.error);
            });

            const sel = this.dom.querySelector('#caracPadreId');
            const opt = document.createElement('option');
            opt.value = d.id; opt.textContent = d.nombre; sel.appendChild(opt);

            this.dom.querySelector('#caracNombre').value = '';
            this.dom.querySelector('#msgAdmin').innerHTML = alerta('Característica agregada.', 'success');
        });

        // Reporte PDF
        this.dom.querySelector('#reporteForm').addEventListener('submit', (e) => {
            e.preventDefault();
            const mes  = this.dom.querySelector('#mes').value;
            const anio = this.dom.querySelector('#anio').value;
            window.open(`/api/admin/reporte/puestos?mes=${mes}&anio=${anio}`, '_blank');
        });
    }
}

function vistaAdminPanel() {
    app().innerHTML = '';
    app().appendChild(new AdminPanelView().dom);
}
