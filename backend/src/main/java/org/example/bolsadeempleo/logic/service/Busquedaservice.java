...
public class Busquedaservice {
    ...
    @Transactional(readOnly = true)
    public List<Puesto> getUltimos5PuestosPrivados() {
        List<Puesto> puestos = puestoRepository.findTop5ByActivoAndPublicaOrderByFechaPublicacionDesc(true, false);
        puestos.forEach(p -> {
            if (p.getEmpresa() != null) p.getEmpresa().getNombre();
            p.getPuestoCaracteristicas().forEach(pc -> {
                if (pc.getCaracteristica() != null) pc.getCaracteristica().getId();
    ...
    @Transactional(readOnly = true)
    public List<Puesto> getUltimos5Puestos() {
        List<Puesto> puestos = puestoRepository.findTop5ByActivoOrderByFechaPublicacionDesc(true);
        puestos.forEach(p -> {
            if (p.getEmpresa() != null) p.getEmpresa().getNombre();
            p.getPuestoCaracteristicas().forEach(pc -> {
                if (pc.getCaracteristica() != null) pc.getCaracteristica().getId();
    ...
    @Transactional(readOnly = true)
    public List<Puesto> buscarTodosLosPuestos(String palabra, BigDecimal salarioMin, Long caracteristica) {
        List<Puesto> puestos = puestoRepository.findByActivo(true);
        puestos.forEach(p -> {
            if (p.getEmpresa() != null) p.getEmpresa().getNombre();
            p.getPuestoCaracteristicas().forEach(pc -> {
                if (pc.getCaracteristica() != null) pc.getCaracteristica().getId();
    ...
    @Transactional(readOnly = true)
    public List<Puesto> buscarPuestosPrivados(String palabra, BigDecimal salarioMin, Long caracteristica) {
        List<Puesto> puestos = puestoRepository.findByActivoAndPublica(true, false);
        puestos.forEach(p -> {
            if (p.getEmpresa() != null) p.getEmpresa().getNombre();
            p.getPuestoCaracteristicas().forEach(pc -> {
                if (pc.getCaracteristica() != null) pc.getCaracteristica().getId();
