...
export default function OferenteHabilidades() {
    ...
    const agregar = async (e) => {
        e.preventDefault();
        const { ok, data } = await apiPost('/api/oferente/habilidades/agregar', {
            caracteristicaId: caracId, nivel
        });
        if (!ok) { setMsg({ tipo: 'error', texto: data.error }); return; }

        setHabilidades(prev => {
            const existe = prev.find(h => h.caracteristica?.id == caracId);
            if (existe) {
                return prev.map(h => h.caracteristica?.id == caracId
                    ? { ...h, nivel: parseInt(nivel) }
                    : h);
            }
            const caract = caracteristicas.find(c => c.id == caracId);
                        ...
                        <div className="form-row">
                            ...
                            <div className="form-group" style={{ flex: 1 }}>
                                ...
                                <input type="number" min="1" max="5" value={nivel}
                                       onChange={e => setNivel(e.target.value)} required />
                            </div>
                            <div className="form-group" style={{ flex: 1, justifyContent: 'flex-end', paddingTop: 22 }}>
                                <button type="submit" className="btn-submit" style={{ margin: 0, padding: '10px 18px' }}>
                                    + Agregar
                                </button>
                            </div>
                        </div>
                        <p style={{ fontSize: 15, color: '#888', marginTop: -8 }}>
                            Nivel 1 = básico · 3 = intermedio · 5 = experto. Si la habilidad ya existe, se actualizará el nivel.
                        </p>
                    </form>
                </div>
                    ...
                    {habilidades.length === 0
                        ...
                        : (
                            <table className="panel-table">
                                <thead>
                                    <tr><th>Característica</th><th>Categoría padre</th><th>Nivel</th><th>Acción</th></tr>
                                </thead>
                                <tbody>
                                    {habilidades.map(h => (
                                        <tr key={h.id}>
                                            ...
                                            <td>
                                                {h.caracteristica?.parent
                                                    ? <span className="badge">{h.caracteristica.parent.nombre}</span>
